...
import { reviewDisposition, RP03 } from './repairReview';
...
export type RepairPolicyItem = { profile: string; sheet: string; targets: string[]; anchor?: string; before_formula?: string };
...
function proposalText(profile: string | undefined, finding: Finding) {
  const range = finding.formula_pattern?.pattern_subtype === 'RANGE_BOUNDARY_DRIFT';
  const constant = finding.formula_pattern?.pattern_subtype === 'CONSTANT_OVERRIDE_CANDIDATE';
  if (profile === RP01) return {
    title: '문자로 저장된 숫자를 계산에 포함하기',
    explanation: '표시된 문자를 숫자로 바꾸면 합계에 포함되는 값이 달라질 수 있습니다. 금액·개수인지 확인한 뒤 실제 영향을 계산합니다.',
  };
  if (profile === RP02) return {
    title: '비어 있는 계산 칸을 같은 방식으로 채우기',
    explanation: '주변의 계산 칸을 기준으로 제안합니다. 같은 업무 계산을 해야 하는 빈 칸인지 확인해야 합니다.',
  };
  if (profile === RP03) return {
    title: '월별 수식 후보 검증하기',
    explanation: '서버가 원본 수식, 계산 오류, 앞뒤 월 계산값을 다시 확인합니다. 이 화면은 후보만 표시하며 지원되지 않는 경우 서버가 거절합니다.',
  };
  if (range) return { title: '합계·참조 범위의 차이 확인', explanation: '어느 범위가 업무에 맞는지 추가 판단이 필요합니다. 현재는 기존 SUM 범위를 바꾸거나 변경 후 값을 계산하는 제안을 제공하지 않습니다.' };
  if (constant) return { title: '수식 대신 입력한 고정값 확인', explanation: '직접 입력한 조정값일 수 있습니다. 현재 엔진은 값이 있는 셀을 수식으로 덮어쓰지 않습니다.' };
  return { title: finding.title, explanation: '현재 규칙으로 수정안을 만들 수 없습니다. 무료 진단은 위치·근거를 확인할 수 있습니다.' };
}

export function proposalGroups(findings: Finding[]): RepairProposal[] {
  const result = new Map<string, RepairProposal>();
  for (const f of findings) {
    const profile = reviewDisposition(f).profile;
    const sheet = f.sheet ?? '통합문서';
    const column = f.cell?.match(/^[A-Z]+/)?.[0] ?? '';
    const groupingCell = profile === RP03 ? f.cell ?? column : column;
    const id = JSON.stringify([sheet, groupingCell, profile ?? f.formula_pattern?.pattern_subtype ?? f.rule_code]);
    let group = result.get(id);
    if (!group) {
      group = { id, sheet, column, profile, findings: [], ...proposalText(profile, f) };
      result.set(id, group);
    }
...
export function intentVerdict(intent: RepairIntent | undefined, detail: PlanDetail, policy?: RepairPolicy) {
  ...
  if (intent.kind === 'same_formula') {
    ...
    return matches ? { status: 'matched' as const, message: `${intent.sheet} ${intent.cell}에 ${intent.anchor}의 계산 방식을 적용한 제안입니다. 행에 맞게 참조를 옮겨 계산했습니다.` }
      : { status: 'unverified' as const, message: '요청한 기준 셀과 대상이 이번 제안에 없습니다. 같은 계산 방식 적용은 실제 빈 셀 복원에서만 확인할 수 있습니다.' };
  }
  ...
  if (!actual || actual.type !== 'number' || canonicalDecimal(actual.value) === null) return { status: 'unverified' as const, message: '이번 변경계획에 해당 위치의 검증된 숫자 결과가 없습니다. 요청을 충족한다고 판단하지 않습니다.' };
  return canonicalDecimal(actual.value) === expected
    ...
    : { status: 'mismatch' as const, message: `요청한 ${intent.expected}와 계산 결과 ${String(actual.value)}가 다릅니다. 요청에 맞추려고 값이나 수식을 임의로 바꾸지 않습니다.` };
}

...
export function RepairPlanPreview({job,onJob}:{job:DeliveryJob;onJob:(job:DeliveryJob)=>void}){
 ...
 useEffect(()=>{if(!detail||job.status==='READY')return;const timer=setTimeout(()=>{setDetail(null);deliveryRequest<DeliveryJob>({action:'get',job_id:job.job_id}).then(onJob).catch(()=>setError('계획이 만료되었습니다. 새 계획을 확인하세요.'))},Math.max(0,detail.expires_at*1000-Date.now()));return()=>clearTimeout(timer)},[detail?.digest,detail?.expires_at,job.job_id,job.status,onJob]);
 const plan=job.plan_summary;
 return <section className="delivery-plan delivery-step" aria-label="변경계획 계산">
  {job.status==='READY'&&<RepairDelivery job={job} detail={detail??{digest:'',patches:[],impact:[],expires_at:job.expires_at}} onJob={onJob}/>}
  <details className="delivery-plan-review" open={job.status!=='READY'}><summary>{job.status==='READY'?'승인한 변경과 계산 근거 다시 보기':'변경 범위·계산 근거 확인'}</summary><h3>변경계획 계산</h3>
  <p>선택한 변경을 사본에서 함께 계산해 전체 수식과 영향을 확인합니다. 아직 원본을 수정하거나 변경을 승인하지 않습니다.</p>
  <button type="button" className="button button--primary" disabled={busy||['APPROVED','RUNNING','CANCEL_REQUESTED','READY'].includes(job.status)} onClick={()=>run(async signal=>{
  ...
  {plan&&<div className="delivery-plan-summary" role="status"><h4>{plan.status==='PREVIEW_VALIDATED'?'변경계획 계산 검증 완료':'계산 완료 · 기준 엔진 검증 필요'}</h4>
   ...
   {(job.internal_rehearsal||job.entitlement_active)&&<>
  ...
  {plan&&job.status!=='READY'&&<OrderStatus job={job} onRefresh={async()=>onJob(await deliveryRequest<DeliveryJob>({action:'get',job_id:job.job_id}))}/>}
  ...
  {detail&&job.status!=='READY'&&(job.internal_rehearsal||job.entitlement_active)&&<RepairDelivery job={job} detail={detail} onJob={onJob}/>}

...
export const formulaAuditCoverageNotice = '지원 범위: 같은 열의 주변 A1 참조 수식 패턴, 제한된 같은 행의 상대 A1 산술 참조 이탈, M01~M12 머리글이 같은 이름의 월별 시트와 대응되는 단순 뺄셈 수식만 비교합니다. Excel Table 내부 수식과 가로 빈 셀·상수는 비교하지 않습니다. 함수·범위·고정참조·외부참조·이름/구조화 참조는 이번 가로 산술·월별 비교에서는 지원하지 않습니다.';
...
export const formulaAuditStatusCopy: Record<FormulaAuditStatus, { label: string; description: string }> = {
  ...
  SKIPPED_UNSUPPORTED_STRUCTURE: {
    label: '미지원 구조로 생략',
    description: '지원하지 않는 수식 구조는 안전하게 후보화하지 않습니다. M01~M12와 다른 월별 시트 패턴, Excel Table 내부 수식, 가로 빈 셀·상수는 이번 수식 패턴 검사 범위에 포함되지 않습니다. 함수·범위·고정참조·외부참조·이름/구조화 참조는 이번 가로 산술·월별 비교에서는 지원하지 않습니다.',
  },
  FAILED: {
        ...
        <div className="formula-audit-panel">
          ...
          {auditResult && (
            <>
              ...
              {auditResult.status === 'COMPLETED' && auditResult.candidates.length === 0 && (
                <p className="formula-audit-panel__empty">이번 지원 범위 안에서는 수식 패턴 후보를 만들지 않았습니다. M01~M12 단순 뺄셈 대응을 벗어난 월별 시트 패턴, Excel Table 내부 수식, 가로 빈 셀·상수처럼 범위 밖인 구조는 0건 완료로 해석하지 말고 별도 확인이 필요합니다. 파일의 모든 수식, 계산 결과, 업무 규칙이 안전하다는 뜻은 아닙니다.</p>
              )}

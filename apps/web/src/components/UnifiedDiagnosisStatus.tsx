...
import {
  formulaAuditCoverageNotice,
  formulaAuditRegionMetricLabel,
  formulaAuditSheetMetricLabel,
  formulaAuditStatusCopy,
  formulaAuditWorkbookSafetyNotice,
...
export function integratedAuditState(audit: IntegratedFormulaAudit) {
  ...
  if (audit.result?.status === 'COMPLETED') return {
    ...
    detail: `${formulaAuditCoverageNotice} 패턴 차이는 검토 후보이며, 잘못된 수식이나 업무 정답을 확정하지 않습니다. ${formulaAuditWorkbookSafetyNotice}`, complete: true,
  };
...
export function UnifiedDiagnosisStatus({ audit, truncated, compact = false, children }: { audit: IntegratedFormulaAudit; truncated: boolean; compact?: boolean; children?: ReactNode }) {
  ...
  return <section className="diagnosis-status" aria-label="검사 진행과 범위">
    ...
    <details className="diagnosis-status__scope">
      ...
      <p>{formulaAuditCoverageNotice}</p>
      <p>구조 위험과 수식 검토 후보를 한 목록에서 보여줍니다. 기존 구조 위험 점수·CSV·직접 재검사 비교에는 구조 검사 결과만 반영합니다.</p>
      ...
      {audit.result && <p>수식 패턴 검사: 수식 셀 {audit.result.formula_cell_count.toLocaleString('ko-KR')}개 · {formulaAuditSheetMetricLabel} {audit.result.audited_sheet_count}개 · {formulaAuditRegionMetricLabel} {audit.result.audited_formula_region_count}개</p>}
      <p>원본 수식 비교는 선택한 파일을 이 브라우저에서만 읽어 표시합니다. 파일에 저장된 결과는 재계산한 값이 아닙니다. 수정안의 예상 결과는 지원 범위·계산 검증을 거친 뒤 3단계에서 확인합니다.</p>
      {children}
    </details>
  </section>;
}

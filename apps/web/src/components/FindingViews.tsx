...
import type { Finding, FindingUserStatus, Severity } from '../types';
...
export function FindingViews({ findings, allFindings, mode, statuses, renderFinding, renderGuidance, onReviewGroup, groupTitle, categoryLabel }: Props) {
  ...
  return <div className="finding-list">{[...groups.entries()].map(([rule, leaves]) => {
    const severity = leaves.some((finding) => finding.severity === 'critical') ? 'critical'
      : leaves.some((finding) => finding.severity === 'warning') ? 'warning' : 'info';
    return (
    <section className="finding-group" key={rule} aria-label={`${rule} 유형`}>
      ...
      <details className="finding-group__locations">
        <summary>개별 위치 {leaves.length}개 보기</summary>

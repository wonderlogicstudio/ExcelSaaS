...
export function ProposalExample({ detail, job, intent }: { detail: PlanDetail; job: DeliveryJob; intent?: RepairIntent }) {
  const preferred = intent?.enabled && detail.patches.some(p => p.sheet === intent.sheet && p.cell === intent.cell) ? intent.cell : job.policy?.targets[0];
  const patch = detail.patches.find(p => p.cell === preferred && (!job.policy || p.sheet === job.policy.sheet)) ?? detail.patches[0];
  ...
  return <section className="proposal-example" aria-label="대표 수정 예시"><span className="section-kicker">검증한 제안 · 아직 원본 변경 없음</span><h3>예를 들면 이렇게 바뀝니다</h3>
    ...
    <p>{patch.after.type === 'formula' ? '선택한 기준 수식을 해당 행으로 옮긴 후 지원 엔진에서 실제로 계산한 값입니다.' : '문자 표기를 계산할 수 있는 숫자 타입으로 바꾼 예시입니다. 합계 등에 미치는 영향도 함께 검사했습니다.'}</p>
    <p>수정할 위치 {detail.patches.length}곳 · 함께 달라지는 계산 결과 {indirect.length}곳. 이 예시 하나가 전체 변경 목록은 아닙니다.</p>

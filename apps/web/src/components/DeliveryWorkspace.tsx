...
const RP01='RP01_NUMERIC_TEXT_FIELD_V1';const RP02='RP02_APPROVED_FORMULA_RESTORE_V1';const COMBINED='COMBINED_RP01_RP02_REPAIR_V1';
...
export function DeliveryWorkspace({ file, initialJob, reviewDraft, onSourceFixed, compactEntry = false, guidedStep, onProgress, onNextStep, intent, onRestartProposal }: {
  ...
  useEffect(() => { const p = initialJob?.policy; if (p && !('items' in p)) { setProfile(p.profile); setSheet(p.sheet); setTargets(p.targets.join(', ')); setRole(p.role ?? ''); setAnchor(p.anchor ?? ''); setFormula(p.anchor_formula ?? ''); setConfirmed(p.confirmed === true); } }, [initialJob]);
  useEffect(() => {
  ...
  const restartProposalSelection = () => {
    abort.current?.abort(); abort.current = null; setBusy(false); setError(null); setOpen(false); setDirty(false); setConfirmed(false); setIntentSatisfied(!intent?.enabled);
    appliedDraft.current = '';
    setJob(current => current ? { ...current, preflight: null, plan_summary: null, approval_status: undefined, repair_execution_available: false, delivery: null } : current);
    onRestartProposal?.();
  };
  ...
  const applyDraft = async (baseJob:DeliveryJob, draft:RepairDraft, signal:AbortSignal, epoch:number) => {
    const policy = draftPolicy(draft);
    const checked = await deliveryRequest<DeliveryJob>({ action: 'preflight', job_id: baseJob.job_id, revision: baseJob.revision, source_hash: baseJob.source_hash, policy }, signal);
    if (signal.aborted) return;
    setOpen(true); setJob(checked); setDirty(false);
  ...
  const draftKey = reviewDraft ? JSON.stringify(reviewDraft) : '';
  const appliedDraft = useRef('');
  useEffect(() => {
    if (!reviewDraft) return;
    if (job && !reviewDraft.proposal) return;
    const first=reviewDraft.items?.[0]??reviewDraft; setProfile(first.profile); setSheet(first.sheet); setTargets(first.targets.join(', ')); setConfirmed(reviewDraft.confirmed === true); setRole(first.role ?? ''); setAnchor(first.anchor ?? ''); setFormula(first.anchor_formula ?? '');
    if (!job) { void start(); return; }
    if (!reviewDraft.proposal || appliedDraft.current === draftKey || ['RUNNING','CANCEL_REQUESTED','READY'].includes(job.status)) return;
    appliedDraft.current = draftKey; const epoch = viewEpoch.current;
    void run(signal => applyDraft(job, reviewDraft, signal, epoch));
  }, [draftKey, job?.job_id, job?.revision]);
  const upload = () => { const epoch = viewEpoch.current; return run(async signal => {
    ...
    if (signal.aborted) return;
    setJob(next); setSheet((reviewDraft?.items?.[0]??reviewDraft)?.sheet ?? next.sheets[0]?.name ?? ''); setDirty(false); onSourceFixed?.(true);
    if (reviewDraft?.proposal && confirmed) {
      appliedDraft.current = JSON.stringify(reviewDraft);
      await applyDraft(next, reviewDraft, signal, epoch);
    }
  }); };
  ...
  return <section id="repair-preflight" className="delivery-workspace shell" aria-label="선택한 항목의 수정 가능 여부">
    <div hidden={!scopeVisible}>
      ...
      {!job ? <div className="delivery-step"><h3>검사할 원본 확인</h3>{reviewDraft?.proposal && <p>선택한 {reviewDraft.items?.length ? `${reviewDraft.items.length}개 묶음 · ${reviewDraft.items.reduce((n,item)=>n+item.targets.length,0)}곳` : `${reviewDraft.sheet}의 ${reviewDraft.targets.length}곳`}을 기존 규칙으로 검사합니다. 지원 조건을 통과하면 변경안을 계산합니다. 아직 수정하지 않습니다.</p>}<p>{file?.name ?? '고정된 합성 원본'} · {maxBytes === null ? '지원 한도 확인 중' : `최대 ${maxBytes / 1024 / 1024}MiB`}</p>
    ...
    {job && onRestartProposal && <button type="button" className="button button--ghost proposal-restart" disabled={busy || ['APPROVED','RUNNING','CANCEL_REQUESTED','READY'].includes(job.status)} onClick={restartProposalSelection}>다른 제안으로 다시 선택</button>}

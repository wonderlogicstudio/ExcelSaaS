...
export function DeliveryWorkspace({ file, initialJob, reviewDraft, onSourceFixed, compactEntry = false, guidedStep, onProgress, onNextStep }: {
  compactEntry?: boolean; file?: File; initialJob?: DeliveryJob; reviewDraft?: RepairDraft; onSourceFixed?: (fixed: boolean) => void;
  ...
  useEffect(() => {
    ...
    onProgress?.({ approvalAvailable: usable && !!(job?.internal_rehearsal || job?.entitlement_active || job?.status === 'READY'),
      deliveryAvailable: usable && !!(job?.approval_status === 'APPROVED' || job?.status === 'READY'), ready: job?.status === 'READY' });
  }, [job?.job_id, job?.status, job?.plan_summary?.digest, job?.plan_summary?.status, job?.internal_rehearsal, job?.entitlement_active, job?.approval_status, dirty, onProgress]);
  ...
  return <section id="repair-preflight" className="delivery-workspace shell" aria-label="선택한 항목의 수정 가능 여부">
    <div hidden={!scopeVisible}>
      <h2 ref={heading} tabIndex={-1}>선택한 항목을 고칠 수 있는지 확인</h2>
      ...
      {!job ? <div className="delivery-step"><h3>검사할 원본 확인</h3><p>{file?.name ?? '고정된 합성 원본'} · {maxBytes === null ? '지원 한도 확인 중' : `최대 ${maxBytes / 1024 / 1024}MiB`}</p>
        ...
        : <><p className="delivery-source-status">원본 확인 완료 · 변경하지 않음 · {job.sheets.length}개 시트</p>
          <details className="delivery-criteria" open={!job.preflight || dirty}><summary>수정 기준과 대상</summary><fieldset className="delivery-step" disabled={busy || ['APPROVED','RUNNING','CANCEL_REQUESTED','READY'].includes(job.status)}><legend>어떤 셀을 어떤 기준으로 바꿀까요?</legend>
    ...
    {job?.preflight?.status === 'PRELIMINARY_ONLY' && !dirty && <RepairPlanPreview job={job} onJob={setJob} guidedStep={guidedStep} onNextStep={onNextStep}/>}

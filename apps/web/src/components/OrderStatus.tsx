...
export function OrderSummary({order}:{order:Order}){
 ...
 <p>{order.artifact_status==='READY'?'같은 주문의 재다운로드에는 추가 결제가 없습니다.':`받을 파일: ${order.artifact_kinds.map(k=>artifactLabels[k]).join(' · ')}`}</p>
 <details><summary>주문과 보관 정보</summary><p>주문 번호: <code>{order.order_id}</code></p><p>입력·계획·결과는 업로드 후 최대 15분, 합성 주문 기록은 24시간의 로컬 임시 보관입니다. 만료 접근은 즉시 차단하고 30초 주기로 정리합니다. 서버 재시작 시 사라질 수 있습니다.</p>{order.input_expires_at&&<p>파일 만료: {new Date(order.input_expires_at*1000).toLocaleString('ko-KR')}</p>}</details></div>;
}
export function OrderStatus({job,onRefresh,limitsConfirmed=false}:{job:{job_id:string;order_id?:string|null;status:string};onRefresh:()=>Promise<void>;limitsConfirmed?:boolean}){
 ...
 return <section className="delivery-step order-panel" aria-label="상품 주문 상태"><h3>주문 상태</h3>

...
export function RepairDelivery({job,detail,onJob,mode='all',onContinue}:{job:DeliveryJob;detail:PlanDetail;onJob:(job:DeliveryJob)=>void;mode?:'all'|'approval'|'delivery';onContinue?:()=>void}){
 ...
 useEffect(()=>{
  ...
  const poll=async()=>{try{onJob(await deliveryRequest<DeliveryJob>({action:'get',job_id:job.job_id},c.signal));}catch(e){if(!c.signal.aborted)setError(e instanceof Error?e.message:'최신 상태를 확인하지 못했습니다.');}finally{if(!c.signal.aborted)timer=setTimeout(poll,1200);}};
 ...
 return <section className="delivery-execution delivery-step" aria-label="변경 승인과 납품"><h3>{approvalOnly?'정확한 변경 승인':mode==='delivery'?'승인한 결과 받기':job.status==='READY'?'수정 패키지 수령':'변경 승인과 납품'}</h3>

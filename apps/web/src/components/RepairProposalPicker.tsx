...
const draftKey=(draft:RepairDraft)=>JSON.stringify([draft.profile,draft.sheet,draft.targets,draft.role??'',draft.anchor??'',draft.anchor_formula??'']);
const basketId=(proposal:RepairProposal)=>proposal.id;
const kindLabel=(profile:string)=>profile===RP01?'숫자 텍스트 정리':'빈 셀 수식 복원';
...
function ProposalDetail({ proposal, file, intent, onPrepare, basketItem, onBasketChange, onBasketRemove }: { proposal: RepairProposal; file: File; intent: RepairIntent; onPrepare: (draft: RepairDraft) => void; basketItem?: BasketItem; onBasketChange: (item: BasketItem) => void; onBasketRemove: (id: string) => void }) {
  ...
  const draft:RepairDraft={ profile: proposal.profile!, sheet: proposal.sheet, targets, role, anchor, anchor_formula: formula?.text ?? '', confirmed: true, proposal: true };
  ...
  return <section className="proposal-details" aria-label={`${proposal.sheet} ${proposal.column}의 제안`}>
    ...
    <details className="proposal-targets"><summary>제안할 위치 선택·제외 ({targets.length}곳 선택)</summary>
      ...
      {proposal.findings.length > 50 && <p>한 번에 표시한 50곳까지 제안합니다. 나머지 위치는 다음 검사에서 선택하세요. 수정 엔진은 최종 시도를 별도 검사합니다.</p>}
    </details>
    {proposal.profile === RP01 ? <fieldset className="proposal-purpose"><legend>이 숫자는 어떤 용도인가요?</legend>
      ...
      <p>주변 수식은 기준 후보입니다. 같은 계산이어야 하는 셀인지 확인하세요. 저장값을 그대로 복사하지 않고 대상 행에 맞게 참조를 옮깁니다.</p>
    ...
    <div className="proposal-action-row">
      ...
      <button className="button button--outline proposal-secondary-preview" type="button" disabled={!canPrepare} onClick={() => onPrepare(draft)}>이 묶음만 변경 예시 확인</button>
    </div>
...
export function RepairProposalPicker({ findings, sheets, file, selection, intent, available, onPrepare }: { findings: Finding[]; sheets: string[]; file: File | null; selection: ReviewSelection; intent: RepairIntent; available: boolean; onPrepare: (draft: RepairDraft) => void }) {
  ...
  const addToBasket=(item:BasketItem)=>setBasket(old=>{setNotice(`${item.title} ${item.cellCount}곳이 목록에 추가됐습니다.`);return [...old.filter(existing=>existing.id!==item.id),item];});
  ...
  const prepareBasket=()=>{if(!basket.length)return;const items=basket.map(item=>item.draft);onPrepare(items.length===1?items[0]:{profile:COMBINED,sheet:items[0].sheet,targets:items.flatMap(item=>item.targets),confirmed:true,proposal:true,items});};
  ...
  return <section id="repair-review" className={`repair-review proposal-picker shell ${basket.length?'has-proposal-basket':''}`} aria-labelledby="proposal-title"><h2 id="proposal-title">어떤 시트의 수정 제안을 볼까요?</h2>
    ...
    <p>후보는 수정 가능 확정이 아닙니다. 선택 후 파일 전체의 보존 조건과 지원 범위를 검사합니다.</p>

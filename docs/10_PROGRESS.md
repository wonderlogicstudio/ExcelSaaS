...
# EFFICIENCY-01 — 운영 범위·중복 축소 적용

사용자 승인에 따라 작업별 검증 범위, 역할·문맥 최소화, 동일 검사 재실행 제한과 두 번 실패 시 중단을 설정했습니다. [운영 규칙](54_AGENT_ORCHESTRATION.md#efficiency-01--사용자-승인-운영-우선-규칙-2026-09-15). 설정 TOML 3개 파싱과 선별 diff 검사만 수행하며 제품·배포·다음 개발 단위는 실행하지 않습니다. 기존 완료/미완료 기록을 유지합니다.

...
| Feedback control (code complete but not deployed) | `FEEDBACK_NOT_CONFIGURED`, `FEEDBACK_RATE_LIMITED`, `INVALID_FEEDBACK_CONTENT_TYPE`, `FEEDBACK_PAYLOAD_TOO_LARGE`, `INVALID_FEEDBACK_PAYLOAD`, `FEEDBACK_STORAGE_UNAVAILABLE` | The small category-only response could not be recorded. The caller can retry only when appropriate. |

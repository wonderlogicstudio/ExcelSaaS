"""Build the guarded HORIZONTAL-01 API overlay image.

This is the RULES-01 image overlay flow with the bounded Horizontal01 delta:
...
BASE = Path("C:/Users/JinwonLee/project/DigitalTwin/.tmp/workbookcare-rules01/image")
...
config["history"].append(
    {
        ...
        "created_by": "Approved HORIZONTAL-01 formula pattern overlay",
    }
)
...
result = {
    ...
    "pushed": False,
    "build": "HORIZONTAL-01 formula_patterns.py overlay on immutable RULES-01 private beta image",
}

if args.push:
    if args.confirm_push != "HORIZONTAL01_APPROVED_PUSH":
        raise SystemExit("Refusing registry mutation without --confirm-push HORIZONTAL01_APPROVED_PUSH")
    ...
    with httpx.Client(
        ...
        response = client.put(
            API + "/manifests/horizontal01-" + sha(new_manifest).split(":")[1][:12],

"""Guarded HORIZONTAL-01 Worker/UI release phases.
...
EXPECTED_WORKER = "3b513785-21e8-4efe-8bc3-fcdf9301d0eb"
...
def require_mutation():
    if "--confirm-mutation" not in sys.argv or sys.argv[sys.argv.index("--confirm-mutation") + 1] != "HORIZONTAL01_APPROVED_MUTATION":
        raise SystemExit("Refusing Worker mutation without --confirm-mutation HORIZONTAL01_APPROVED_MUTATION")
...
if phase == "baseline":
    ...
    out = w(
        ...
        *overrides,
        ...
        "horizontal01",
        "--message",
        "Approved Horizontal01 integration UI build; private beta boundaries preserved",
    )
    ...
    assert sha(bindings(v)) == b["binding_hash"]
    assert api() == b["api"]
    w("versions", "deploy", v + "@100", "--yes", "--message", "Approved Horizontal01 integration UI build")
    assert worker() == v
    assert api() == b["api"]

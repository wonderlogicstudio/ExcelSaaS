...
O = Path(__file__).parent / "web-release"
O.mkdir(parents=True, exist_ok=True)
API_RELEASE = WORK / "release"
FREEZE = Path(__file__).parent / "source-freeze.json"
G = r"C:\Users\JinwonLee\AppData\Local\Google\Cloud SDK\google-cloud-sdk\bin\gcloud.cmd"
EXPECTED_WORKER = "7d353c64-457d-4376-922e-03ecbcc4d0bc"
...
def require_mutation():
    if "--confirm-mutation" not in sys.argv or sys.argv[sys.argv.index("--confirm-mutation") + 1] != "MONTHLY_COPY01_APPROVED_MUTATION":
        raise SystemExit("Refusing Worker mutation without --confirm-mutation MONTHLY_COPY01_APPROVED_MUTATION")
...
if phase == "baseline":
    ...
    out = w(
        ...
        *overrides,
        ...
        "monthly-copy01",
        "--message",
        "Approved monthly explanation localization; private beta boundaries preserved",
    )
    ...
    assert sha(bindings(v)) == b["binding_hash"]
    assert api() == b["api"]
    w("versions", "deploy", v + "@100", "--yes", "--message", "Approved monthly explanation localization")
    assert worker() == v
    assert api() == b["api"]

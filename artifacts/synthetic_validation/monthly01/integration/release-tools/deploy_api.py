"""Guarded MONTHLY-01 protected-beta API release phases.

This keeps the proven RULES-01 sequence and only substitutes the Monthly01
...
EXPECTED_BASELINE_REVISION = "workbookcare-api-beta-00026-get"
EXPECTED_BASELINE_IMAGE = (
    "asia-northeast3-docker.pkg.dev/workbookcare-beta/workbookcare-images/"
    "workbookcare-api@sha256:1b0702e02c7b303b25d999d8da68a9a3c7c40a639a593dd3f8c4497fe6f3c596"
)
EXPECTED_WORKER_VERSION = "7cbbadde-3b30-4792-a84f-0543db1c61a9"
...
def require_mutation():
    if "--confirm-mutation" not in sys.argv or sys.argv[sys.argv.index("--confirm-mutation") + 1] != "MONTHLY01_APPROVED_MUTATION":
        raise SystemExit("Refusing cloud mutation without --confirm-mutation MONTHLY01_APPROVED_MUTATION")
...
if phase == "baseline":
    ...
    raise SystemExit("Use reviewed guarded_transition.py for immediate provider preflight and traffic switch")
elif phase == "after":
    b = read("baseline.json")
    live = read("live.json")
    s = service()
    ...
    raise SystemExit("Unknown bounded Monthly01 release phase")

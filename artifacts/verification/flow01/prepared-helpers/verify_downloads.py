...
O = Path("C:/Users/JinwonLee/project/DigitalTwin/.tmp/workbookcare-flow01")
D = O / "browser-downloads"
oracle = delivery.cases.oracle()
results = []
for label in ["RP01_ALL", "RP02_ALL"]:
    ...
    assert ("TARGET_STATIC_FINDINGS_RESOLVED" in codes) == (label == "RP01_ALL")
    assert ("TARGET_FORMULA_CANDIDATES_RESOLVED" in codes) == (label == "RP02_ALL")
...
print(
    json.dumps({"status": "PASS", "downloaded_files": 6, "frozen_inputs": len(hashes)})
)
